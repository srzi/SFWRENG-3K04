...
#include "Group30_SFWR_Assignment1_SIMULINK_private.h"
#include "rtwtypes.h"
#include "limits.h"
...
void *subrateTask(void *arg)
{
  ...
  while (runModel) {
    ...
    if (terminatingmodel)
      break;
    Group30_SFWR_Assignment1_SIMULINK_step(subRateId);
...
void *baseRateTask(void *arg)
{
  runModel = (rtmGetErrorStatus(Group30_SFWR_Assignment1_SIM_M) == (NULL));
  while (runModel) {
    mw_osSignalEventWaitEver(&baserateTaskSem);
    if (rtmStepTask(Group30_SFWR_Assignment1_SIM_M, 1)
        ) {
      mw_osSignalEventRelease(&subrateTaskSem[0]);
    }

    Group30_SFWR_Assignment1_SIMULINK_step(0);

    /* Get model outputs here */
    stopRequested = !((rtmGetErrorStatus(Group30_SFWR_Assignment1_SIM_M) ==
                       (NULL)));
  }
...
{
  UNUSED(sig);
  rtmSetErrorStatus(Group30_SFWR_Assignment1_SIM_M, "stopping the model");
  runModel = 0;
}
...
void *terminateTask(void *arg)
{
  ...
  /* Terminate model */
  Group30_SFWR_Assignment1_SIMULINK_terminate();
...
int main(int argc, char **argv)
{
  ...
  hardware_init();
  rtmSetErrorStatus(Group30_SFWR_Assignment1_SIM_M, 0);

  /* Initialize model */
  Group30_SFWR_Assignment1_SIMULINK_initialize();
